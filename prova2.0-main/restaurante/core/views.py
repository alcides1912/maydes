...
def listar_comidas(request):
    ...
    return render(request, 'core/cardapio/listar_comidas.html', {'comidas': comidas})

def criar_comida(request):
    if request.method == 'POST':
        ...
        form = ComidaForm()
    return render(request, 'core/cardapio/form_comida.html', {'form': form})
...
def excluir_comida(request, pk):
    comida = get_object_or_404(Comida, pk=pk)
    if request.method == 'POST':
        comida.delete()
        return redirect('listar_comidas')
    return render(request, 'core/cardapio/confirmar_exclusao.html', {'comida': comida})
...
def criar_pedido(request):
    if request.method == 'POST':
        ...
        form = PedidoForm()
    return render(request, 'core/cardapio/form_pedido.html', {'form': form})

def listar_pedidos(request):
    pedidos = Pedido.objects.all()
    return render(request, 'core/cardapio/listar_pedidos.html', {'pedidos': pedidos})

from .models import Evento, Comida
from .forms import EventoForm

def listar_eventos(request):
    eventos = Evento.objects.all()
    return render(request, 'core/eventos/listar_eventos.html', {'eventos': eventos})
...
def criar_evento(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('listar_eventos.html')
    else:
        form = EventoForm()
    return render(request, 'core/eventos/form_evento.html', {'form': form})
